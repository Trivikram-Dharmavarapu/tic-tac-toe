...
Game* getGameForObserver(Player* player) {
    string status = "";
...
void handleRefresh(Player* player, string receivedText){
    vector<string> split = splitInputText(receivedText);
    if (split[0] == "refresh" && split.size()==1) {

        if(player->in_game){
            displayGame(player,player->game , "Refresh Done..!\n", true);
        }
        else{
            displayGame(player, *getGameForObserver(player), "Refresh Done..!\n", true);
        }
    }
    else{
    ...
    if (n < 0) {
        ...
        if(player_index != -1) {
            ...
            if(player->valid){
                ...
                else if(receivedText.find("refresh") != string::npos && !player->mail_body){
                    handleRefresh(player,receivedText);
                }
    ...
    for (;;) {
        ...
        for (auto it = next(client_fds.begin()); it != client_fds.end();) {
            ...
            if (FD_ISSET(fd, &readfds)) {
                ...
                try{
                    memset(buf, 0, sizeof(buf));
                    closeConnection = handleClient(fd);
                }
                catch(const exception& e) {
                    int player_index = findPlayer(fd);
                    if(player_index != -1){
                        Player* player = &players[player_index];
                        strcat(buf, "Invalid option or arguments, retry...!");
                        writeToClient(player->fd,strlen(buf), player, false);
                    }
                    cout << "continue "<< endl;
                }
                if(closeConnection){
